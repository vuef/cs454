...
def count(k):
    ...
    print (A)
    #for r in range(1, k):
    #    print np.matmul(A,transition)
    '''uA^kv'''
    #np.matmul(start,A, A)
    #np.matmul(A,accept, A)
    return A
...
def regular(w):
    ...
    copyList = [len(word)+1]
    regEx = []
    # These strings are the inserts.
    for i in range(len(word)+1):
        # Make a copy of the word into a new list.
        copyList = list(word)
        # Insert at a position i an alpha '.'
        copyList.insert(i,'.')
        # Insert into final regular expression array.
        regEx.append(copyList)

    # These strings are the deletions.
    for d in range(0, len(word)):
        copyList = list(word)
        # Delete an element of the array.
        del copyList[d]
        # Append to final regular expression array.
        regEx.append(copyList)
    
    # These strings are the replacements.
    for r in range(0, len(word)):
        copyList = list(word)
        # Replace a letter with an alpha '.'
        copyList[r] = '.'
        # Append to final regular expression array.
        regEx.append(copyList)
    ...
    regularExpression = ''
    first = True
    # Turn regEx array into string for output.
    for reg in range(len(regEx)):
        # Add a plus to represent the union of
        # regular expressions. Only add '+' after
        # the first expression.
        if(not first):
            regularExpression += ' + ' 
        # Iterate through each array of characters
        # and add to string
        for c in regEx[reg]:
            regularExpression += str(c)
        first = False
...
if (__name__ == "__main__"):
    while True:
        i = input("1.count 2.minString 3.regularExpression q.Quit\nEnter a problem number to test: ")
        if(i == "q"):
            break;
        if(int(i) == 1):
